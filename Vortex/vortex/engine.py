...
class WorkflowEngine:
    ...
    async def run(
        ...
        try:
                ...
                for name, result in zip(runnable, results):
                    ...
                    if result.status == AgentStatus.FAILED:
                        if self.logger:
                            self.logger.agent_failed(
                                run.run_id, name, result.error or "", result.duration_ms, result.retry_count
                            )
                        if self.metrics:
                    ...
                    if self.logger:
                        self.logger.agent_complete(
                            run.run_id, name, result.duration_ms, result.retry_count
                        )
                    if self.metrics:
            ...
            if self.logger:
                self.logger.workflow_failed(run.run_id, workflow.name, str(e), wf_duration)
            if self.metrics:
                self.metrics.on_workflow_failed(workflow.name, wf_duration / 1000)
            if self.tracer:
                self.tracer.end_workflow_span(run.run_id, "failed", str(e))
            raise
    ...
    async def _execute_agent(
        ...
        try:
            ...
            if agent_span:
                self.tracer.end_agent_span(agent_span, "timeout", f"timed out after {agent.timeout}s")
            return AgentResult(
                output={},
        ...
        if agent_span:
            self.tracer.end_agent_span(
                agent_span, result.status.value, result.error, result.retry_count
            )
